...
exports.prototype = {
  ...
  initialize: function (_options) {
	...
	this._endpoint = "http://api.sparrowsms.com/v2/sms";
	this._token = "insert_api_token";
	this._from = "insert_registered_sender";
    return this;
  },
  ...
  send: function (_message, _callback) {
	  ...
	  var url = this._endpoint + "?" + Object.keys(params).map(function(key){ 
  ...
  register_receive_handler: function (_handler) {
    var app = express();
    app.get('/api/v1/transport/sparrowsms', function(req, res) {
      from = $_GET["from"]; // sms sender
      // Should include? to = $_GET["to"]; // shortcode
      // Should include? keyword = $_GET["keyword"]; // first word
