...
exports.prototype = {
  ...
  initialize: function (_options) {
    ...
    this._receivePath = '/api/v1/transport/smssync';
    this._sendCallbackPath = '/smssync';
  ...
  _receive_handler: function(req, res) {
      ...
      if (from && message && sent_timestamp && message_id) {
        if (secret == expectedSecret) {
            success = true;
            handlerAfterReceive({from: from, timestamp: sent_timestamp, content: message},
                 function (_err) {
                   if (_err) {
                     console.log(_err) 
                   }
    ...
    _send_callback_handler: function(request, response) {
  ...
  register_receive_handler: function (_handler) {
    ...
    handlerAfterReceive = _handler
    webserver.listenOnPath(this._receivePath, 'post', this._receive_handler)
    return this;
  },
  ...
  start: function () {
    expectedSecret = this._secret
    webserver.listenOnPath(this._sendCallbackPath, ['get', 'post'],
        this._send_callback_handler);
    return this;
  },
